...
Capture cam;
trackColor[] tCols = new trackColor[3];
;
void setup() {
  size(640, 360);
  setupCam();
...
void draw() {
  ...
  for (trackColor tc : tCols) {
    tc.updateImg();
    tc.displayColor(new PVector(10, 10));
  }
}

void mousePressed() {
  if (mouseButton == LEFT) {
    tCols[0].pcolor = cam.get(mouseX, mouseY);
  } else if (mouseButton == RIGHT) {
    tCols[1].pcolor = cam.get(mouseX, mouseY);
  } else {
    tCols[2].pcolor = cam.get(mouseX, mouseY);
  }
}
