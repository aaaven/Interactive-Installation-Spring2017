...
PImage depthImg;
int depthRangeMin = 3, depthRangeMax = 4499, imgScale = 5;
int leftX = 0, rightX = 512, topY = 0, bottonY = 424;
...
void draw() {
  ...
  for (int y = 0; y < depthImg.height; y++) {
    for (int x = 0; x < depthImg.width; x++) {
      ...
      ptCould[index] = new PVector(x, y, rawDepth[index]);
      ...
      if (inRange) {
        ...
        point(imgScale*ptCould[index].x, imgScale*ptCould[index].y, ptCould[index].z);
      }
    }
  }
